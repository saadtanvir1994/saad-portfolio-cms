...
const AnimatedCTAButton: React.FC<AnimatedCTAButtonProps> = ({
  ...
  if (variant === 'simple' || variant === 'simpleoutlined') {
    return (
      <Link
        href={href}
        className={`flex w-fit items-center justify-center gap-2 rounded-full px-6 py-3 text-base font-medium transition-all duration-300 ${variantClasses[variant]} ${align === 'right' ? 'flex-row' : 'flex-row-reverse'} ${className}`}
        aria-label={ariaLabel}
      >
        {text && (
  ...
  return (
    <Link
      href={href}
      className={`group inline-flex w-fit items-center gap-2 rounded-full px-8 py-4 text-base font-medium transition-all duration-300 ease-in-out hover:scale-105 ${variantClasses[variant]} ${align === 'right' ? 'flex-row' : 'flex-row-reverse'} ${className}`}
      aria-label={ariaLabel}
    >
      {text && <span className="text-md font-semibold uppercase">{text}</span>}
      <span
        className={`relative flex h-8 w-8 items-center justify-center rounded-full transition-all duration-300 ease-in-out group-hover:scale-110 ${variant === 'light' ? 'bg-[#FF4102]' : 'bg-white'} overflow-hidden`}
      >
        <span
          className={`h-2 w-2 rounded-full transition-all duration-300 ease-in-out group-hover:scale-0 group-hover:opacity-0 ${variant === 'light' ? 'bg-white' : 'bg-[#FF4102]'}`}
        />
        <MoveRight
          className={`absolute h-4 w-4 translate-y-8 transition-all duration-300 ease-in-out group-hover:translate-y-0 ${variant === 'light' ? 'text-white' : 'text-gray-800'}`}
        />
      </span>
    </Link>
  );
